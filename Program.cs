...
class Program
{
    ...
    {
        Console.WriteLine("\n File Parser tool - ver 1.0. - release .01 - build 100");
        Console.WriteLine(" Mechanical tools (c) - All rights reserved! (2024).\n");
        Console.WriteLine("  -createjson or -createfile to create examples files!");
        Console.WriteLine("  -help or -h to show help!\n");

        if (args.Length == 0 || args[0] == "-help" || args[0] == "-h")
        {
            Console.WriteLine("  Usage: FileParse <action> <filename>");
            Console.WriteLine("  Action: parse");
        ...
        {
            Console.WriteLine("  Usage: FileParse json fileparse.json\n");
            Console.WriteLine("  A JSON file called FileParse.json will be create in the current folder");

            var examplejsonfile = new JSonToParse
            {
                Action = "Parse",
                ParseFileName = "FileParse.dll"
            };
            string examplejsonString = JsonSerializer.Serialize(examplejsonfile);

            try
            {
                File.WriteAllText("fileparse.json", examplejsonString);
        ...
        {
            Console.WriteLine("  Usage: FileParse file fileparse.batch\n");
            Console.WriteLine("  A file called FileParse.batch will be create in the current folder");

            string[] examplebatchfile = ["fileparse.exe", "fileparse.dll", "fileparse.json"];
            try
            {
                File.WriteAllLines("fileparse.batch", examplebatchfile);
                Console.WriteLine("  Created Successfully");
            }
            catch (Exception e)
            {
        ...
        if (args[0] == "parse")
        {
            if (args.Length == 2)
            {
                ...
                {
                    Console.WriteLine(" File: " + args[1] + " will be verified!");
                    FileParsing(args[1]);
            ...
            {
                Console.WriteLine(" File name not informed!");
                Console.WriteLine(" Please verify!");
                // 
                Environment.Exit(0);
            }
        ...
        if (args[0] == "json")
        {
            if (args.Length == 2)
            {
                if (File.Exists(args[1]))
                {
                    Console.WriteLine(" JSON File: " + args[1] + " will be verified!\n");

                    using FileStream JSONParseFile = File.OpenRead(args[1]);
                    JSonToParse? jsonParsing = JsonSerializer.Deserialize<JSonToParse>(JSONParseFile);

                    Console.WriteLine($" Action: {jsonParsing?.Action}");
                    Console.WriteLine($" Filename: {jsonParsing?.ParseFileName}");

                    if (File.Exists(jsonParsing?.ParseFileName))
                    {
                        FileParsing(jsonParsing.ParseFileName);
                    }
                    else
                    {
                        Console.WriteLine(" File: " + jsonParsing?.ParseFileName + " Not found!");
                        Console.WriteLine(" Please verify!");
                        // 
                        Environment.Exit(0);
                    }
                ...
                {
                    Console.WriteLine(" JSON File: " + args[1] + " Not found!");
                    Console.WriteLine(" Please verify!");
                    // 
                    Environment.Exit(0);
                }
            ...
            {
                Console.WriteLine(" JSON File not informed!");
                Console.WriteLine(" Please verify!");
                // 
                Environment.Exit(0);
            }
        ...
        if (args[0] == "file")
        {
            if (args.Length == 2)
            {
                if (File.Exists(args[1]))
                {
                    Console.WriteLine(" Batch File: " + args[1] + " will be verified!\n");

                    var BatchParseFile = File.ReadAllLines(args[1]);

                    if (BatchParseFile.Length == 0)
                    {
                        Console.WriteLine(" Batch File is Empty!");
                        Console.WriteLine(" Please verify!");
                        // 
                        Environment.Exit(0);
                    }

                    for(int i=0; i < BatchParseFile.Length; i++)
                    {

                        //Console.WriteLine(" Filename: " + BatchParseFile[i]);
                        if (File.Exists(BatchParseFile[i]))
                        {
                            Console.WriteLine(" File: " + BatchParseFile[i] + " will be parsed!");
                            FileParsing(BatchParseFile[i]);
                        }
                        else
                        {
                            Console.WriteLine(" File: " + BatchParseFile[i] + " Not found!");
                            Console.WriteLine(" Please verify!");
                            // 
                            Environment.Exit(0);
                        }
                ...
                {
                    Console.WriteLine(" Batch File: " + args[1] + " Not found!");
                    Console.WriteLine(" Please verify!");
                    // 
                    Environment.Exit(0);
                }
            ...
            {
                Console.WriteLine(" Batch File not informed!");
                Console.WriteLine(" Please verify!");
                // 
                Environment.Exit(0);
            }
    ...
    static void FileParsing(string argfilename)
    {

        FileInfo fileparsed = new FileInfo(argfilename);
        FileVersionInfo fileparseinfo = FileVersionInfo.GetVersionInfo(argfilename);

        Console.WriteLine(" Extension: " + fileparsed.Extension.ToUpper());
        Console.WriteLine(" Mime Type: " + GetMimeType(fileparsed.Extension.ToUpper()));
        Console.WriteLine(" Size: " + (fileparsed.Length / 1024) + " KBytes.");
        Console.WriteLine(" Directory: " + fileparsed.DirectoryName);
        Console.WriteLine(" Created:  " + fileparsed.CreationTime);
        Console.WriteLine(" Accessed: " + fileparsed.LastAccessTime);
        Console.WriteLine(" Modified: " + fileparsed.LastWriteTime);
        Console.WriteLine(" Attribute: " + fileparsed.Attributes);
        Console.WriteLine(" Is Read Only? " + fileparsed.IsReadOnly);
        Console.WriteLine(" Hash code: " + fileparsed.GetHashCode());
        Console.WriteLine(" MD5 Hash: " + CalculateMD5(argfilename));
        Console.WriteLine("  Comments: " + fileparseinfo.Comments);
        Console.WriteLine("  Description: " + fileparseinfo.FileDescription);
        Console.WriteLine("  Version: " + fileparseinfo.ProductVersion);
        Console.WriteLine("  Trademarks: " + fileparseinfo.LegalTrademarks);
        Console.WriteLine("  Copyright: " + fileparseinfo.LegalCopyright);
        Console.WriteLine("  Build: " + fileparseinfo.SpecialBuild + "\n");
